...
2. El workflow, solo:
   ...
   - Bumpea la versión en `api/package.json` y `client/package.json`.
   - Abre el PR `release/X.Y.Z → master`.
...
- `master` se mergea de vuelta a `develop`, para que no se desincronicen.
